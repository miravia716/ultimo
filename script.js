...
function showQuestion() {
  answered = false;
  const questionObj = triviaData[currentCategory][currentQuestionIndex];
...
function selectAnswer(selectedIndex) {
  ...
  buttons.forEach((button, index) => {
    ...
    if (index === selectedIndex && selectedIndex !== questionObj.answer) {
  ...
  if (selectedIndex === questionObj.answer) {
    score++;
    resultP.textContent = '¡Respuesta correcta!';
  } else {
    resultP.textContent = 'Respuesta incorrecta.';
  }
...
nextBtn.onclick = () => {
  ...
  resultP.textContent = '';
  if (currentQuestionIndex < triviaData[currentCategory].length) {
    showQuestion();
  } else {
    showResults();
  }
